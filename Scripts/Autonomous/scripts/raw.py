...
cv2.createTrackbar('Hue min', 'Colour Filters', h_low, 0xFF,
                   empty_callback)
cv2.createTrackbar('Hue max', 'Colour Filters', h_high, 0xFF,
                   empty_callback)
cv2.createTrackbar('Sat min', 'Colour Filters', s_low, 0xFF,
                   empty_callback)
cv2.createTrackbar('Sat max', 'Colour Filters', s_high, 0xFF,
                   empty_callback)
cv2.createTrackbar('Bgt min', 'Colour Filters', v_low, 0xFF,
                   empty_callback)
cv2.createTrackbar('Bgt max', 'Colour Filters', v_high, 0xFF,
                   empty_callback)
cv2.createTrackbar('0 : OFF \n1 : ON', 'Colour Filters', switch, 1,
                   empty_callback)
cv2.createTrackbar('1 Circle \nAll Circles', 'Colour Filters',
                   num_circle, 1, empty_callback)
        ...
        if not circles is None:
            ...
            for i in circles[0, :]:
                if i[2] > biggest:
                    ...
                    big_circle = i
            if num_circle == 1:
                cv2.line(frame, (big_circle[0], 1), (big_circle[0],
                         640), 5000, 2)
                cv2.line(frame, (1, big_circle[1]), (640,
                         big_circle[1]), 5000, 2)
            else:
                for i in circles[0, :]:
                    ...
                    cv2.circle(frame, (i[0], i[1]), i[2], (0, 0xFF, 0),
                               -1)
