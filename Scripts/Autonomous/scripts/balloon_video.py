...
class BalloonVideo:
    ...
    def __init__(self):
        ...
        self.camera_type = balloon_config.config.get_integer('camera',
                'type', 0)
        ...
        self.img_width = balloon_config.config.get_integer('camera',
                'width', 640)
        self.img_height = balloon_config.config.get_integer('camera',
                'height', 480)
        ...
        self.cam_hfov = balloon_config.config.get_float('camera',
                'horizontal-fov', 70.42)
        self.cam_vfov = balloon_config.config.get_float('camera',
                'vertical-fov', 43.3)

        # define video output filename

        self.video_filename = balloon_config.config.get_string('camera'
                , 'video_output_file', '~/balloon-%Y-%m-%d-%H-%M.avi')
    ...
    def open_video_writer(self):
        ...
        self.video_writer = cv2.VideoWriter(self.video_filename, ex,
                25, (self.img_width, self.img_height))
        if not self.video_writer is None:
            print('started recording video to %s' % self.video_filename)
        else:
            print('failed to start recording video to %s' \
                % self.video_filename)
    ...
    def pixels_to_angle_y(self, num_pixels):
        return num_pixels * math.radians(self.cam_vfov) \
            / self.img_height
    ...
    def angle_to_pixels_y(self, angle):
        return int(angle * self.img_height
                   / math.radians(self.cam_vfov))
